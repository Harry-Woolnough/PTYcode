...
data<- read.csv("DMPsPathology_rTg4510_sig_pathology.csv")
...
gr4<-merge(gr3, df2, by = intersect(names(gr3), names(df2)), all.y = TRUE)
gr4<-gr4[-c(4,6,7)]
gr5<-(gr4[!duplicated(gr4$start_position),])
data["Gene"]<-gr5["GENE ID"]
...
write.csv(data,"DMPsPathology_rTg4510_sig_pathologyANNOTATIONS.csv")
